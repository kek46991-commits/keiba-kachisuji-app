...
Stripe の競馬予想サービス利用制限を踏まえ、月額購読の主決済を国内決済代行会社
「テレコムクレジット株式会社」へ移行します。正式な審査・契約・項目仕様が確定する
までは、環境変数が未設定でもサイトは決済準備中として正常表示されます。

設定する環境変数:

- `TELECOM_CREDIT_URL_CARD`: クレジットカード月額継続課金ページURL
- `TELECOM_CREDIT_URL_CARRIER`: ドコモ・au・ソフトバンクのキャリア決済（月額）ページURL
- `TELECOM_CREDIT_URL_BANK`: 銀行振込（月額）ページURL
- `TELECOM_CREDIT_URL_PAYPAY`: PayPayの都度購入ページURL（継続課金には使用しない）
- `TELECOM_CREDIT_CHECKOUT_URL`: 後方互換用の汎用URL。`URL_CARD` 未設定時のカードURLとして扱います
- `TELECOM_CREDIT_CALLBACK_SECRET`: 結果通知の共有シークレット

月額プランのCTAには、設定済みのカード・キャリア・銀行振込だけが表示されます。
PayPayは継続課金に対応しないため、設定時も月額プランとは別の「都度購入」として
表示します。決済代行会社の審査前にURLを設定したり、実在しないURLを表示したり
しないでください。
...
- 公開サイト ✅
- 国内銀行口座
- VISA / Mastercard 審査通過
- テレコムクレジットの加盟店審査・契約完了

審査完了後、発行された各決済ページURLと結果通知シークレットを環境変数へ設定し、
`POST /api/telecom/callback` を結果通知先として登録します。テレコムクレジットの
正式な結果通知フィールド名は契約後に確定するため、現実装のフィールド対応は暫定です。
認証・購読状態の保存は既存の subscriber store を再利用します。Stripeの既存実装は
レガシー fallback として残しています。
