...
export default function SyntheticPredictionLabPage() {
  ...
  return <main className="min-h-screen bg-[#08111f] pb-12 text-slate-100">
    <header className="border-b border-white/10 bg-[#0b1628]"><div className="mx-auto flex min-h-16 max-w-5xl items-center gap-3 px-4 py-3"><FlaskConical className="h-5 w-5 text-amber-300" /><div className="flex-1"><h1 className="font-bold">隔離テスト予想ラボ</h1><p className="text-xs text-slate-400">非実在の馬A〜馬Fだけを使うDB保存・表示テスト</p></div><Link href="/admin/odds-simulation" className="text-xs text-cyan-300">EVテストへ</Link></div></header>
    <section className="mx-auto max-w-5xl space-y-5 px-4 pt-7">
      <div className="flex gap-3 rounded-lg border border-amber-300/30 bg-amber-300/10 p-4 text-sm text-amber-50"><FlaskConical className="mt-0.5 h-5 w-5 shrink-0 text-amber-300" /><div><p className="font-semibold">実在レース・公開予想・実績集計から完全分離</p><p className="mt-1 text-amber-100/80">この画面では非実在の識別子だけを保存します。実在馬名への置換、公開レース一覧への表示、成績集計への反映は行いません。</p></div></div>
