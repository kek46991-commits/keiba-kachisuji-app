...
export function parseShutuba(html: string, url: string): ParsedEntry[] {
  ...
  rows.each((_, row) => {
    ...
    entries.push({
      ...
      jockey: text($row.find("td.Jockey a").first().text()) || text($row.find("td.Jockey").first().text()) || null,
      trainer: text($row.find("td.Trainer a").first().text()) || text($row.find("td.Trainer").first().text()) || null,
